...
var __hopServiceName = 'hazard_detection_light_check';
 ...
 */
service hazard_detection_light_check ( {file_uri:'', fast: false} )
{
  ...
  if( file_uri.indexOf(__serverCacheDir) === -1 )
  {
    ...
    console.log(colors.error + '[' + __hopServiceName + ']: ' + errorMsg + colors.clear);
 ...
 *  @returns {Int} response.light_level - Estimated light level.
 *  @returns {String} response.error - Error message string to be filled
 *    when an error has been occured during service call.
 */
function craft_response(rosbridge_msg)
{
  var level = rosbridge_msg.light_level;
